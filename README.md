...
6. Import the provided SQL file without docker. The file is known as SQL_Database_edoc.sql located inside the source code root folder.
7. Import the provided SQL file docker. use this command.
```bash
 cat SQL_Database_edoc.sql | docker exec -i edoc-doctor-appointment-system-mysql-1 /usr/bin/mysql -u root --password=root edoc
```
8. Browse the Doctor's Appointment Systsem in a browser. i.e. http://localhost/edoc-echanneling-main/.
